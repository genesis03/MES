...
def _refresh_order_status(db: Session, order: ProductionWorkOrder | None):
    if order is None:
        return
...
def cancel_in_progress_run(
    ...
    if run.status != "IN_PROGRESS" or run.performance_id is not None:
        raise HTTPException(409, "생산중 가동내역만 취소할 수 있습니다.")
    ...
    return {
        "status": "success",
        "message": "가동내역을 취소했습니다. 취소 이력은 가동내역에 유지됩니다.",
    }
